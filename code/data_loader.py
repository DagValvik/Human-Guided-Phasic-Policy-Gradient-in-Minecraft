            ...
            if ret:
                ...
                if step_data["isGuiOpen"]:
                    camera_scaling_factor = (
                        frame.shape[0] / MINEREC_ORIGINAL_HEIGHT_PX
                    )
                    cursor_x = int(
                        step_data["mouse"]["x"] * camera_scaling_factor
                    )
                    cursor_y = int(
                        step_data["mouse"]["y"] * camera_scaling_factor
                    )
                ...
                output_queue.put(
                    (trajectory_id, frame, action), timeout=QUEUE_TIMEOUT
                )
            else:
...
class DataLoader:
    ...
    def __init__(
        ...
        unique_ids = list(
            set([os.path.basename(x).split(".")[0] for x in unique_ids])
        )
        ...
        for unique_id in unique_ids:
            video_path = os.path.abspath(
                os.path.join(dataset_dir, unique_id + ".mp4")
            )
            json_path = os.path.abspath(
                os.path.join(dataset_dir, unique_id + ".jsonl")
            )
    ...
    def __next__(self):
        ...
        for i in range(self.batch_size):
            workitem = self.output_queues[
                self.n_steps_processed % self.n_workers
